...
# bash synteny_wrapper.sh -g "${HOME}/data/fha1_dbs" -i "${HOME}/data/synteny_input_fha1.tsv" -L "${HOME}/data/hmms_of_interest_fha1.txt" -d "${HOME}/data/hmms_fha1_search/hmms_fha1" -m "${HOME}/data/hmms_fha1_search/hmms_fha1.tsv" &
# for testing:
# bash synteny_wrapper.sh -g "${HOME}/data/genbank_toy/bacteria/" -i "${HOME}/data/synteny_input_fha1.tsv" -L "${HOME}/data/hmms_of_interest_fha1.txt" -d "${HOME}/data/hmms_fha1_search/hmms_fha1" -m "${HOME}/data/hmms_fha1_search/hmms_fha1.tsv"
...
while getopts "g:i:L:d:m:" opt; do
  case $opt in
    g) genome_db="${OPTARG%/}/" ;;  # ensures the path to this dir ends with exactly one slash
    i) input_file="$OPTARG" ;;
    L) hmms_list="$OPTARG" ;; # hmms of interest; only write metadata pertaining to these hmms
...
echo "CLIs: ${genome_db}, ${input_file}, ${hmms_list}, ${hmms_dir}, ${hmms_metadata}" > $log
...
bash "${scriptsdir}/find_synteny_hits.sh" -g $genome_db -i $input_file -L $hmms_list &
