...
print_help() {
    echo "Usage: $0 -b <blast> -s <synteny> -m <multiple> -p <pident> -q <qcovs> -c<'genome'|'locus'> -o <outname> -h"
...
# Parse options
while getopts "b:s:mp:q:c:ko:h" opt; do
  case $opt in
    b) blast="$OPTARG" ;;
    s) synteny="$OPTARG" ;;
    m) multiple="true" ;;
...
currDir=$pwd # save this; especially if running this script in Nextflow, need to be careful about current directory location
...
max=10
blastp -query "$tempSyntenyFasta" -db "${temp_dir}/temp_db" -max_target_seqs "$max" -num_threads "$procs_to_use" -outfmt "6 qseqid qseq sallseqid sseq pident qcovs evalue bitscore" -out "$tempBlastOutputs" &
pid4=$!
...
if [[ $keep = "true" ]]; then
  blastDBname="${blast%.*}_db"
  mv $temp_dir $blastDBname
  echo "Saved blast DB made from input BLAST file to $blastDBname"
else
  rm -rf "$temp_dir"
fi
...
fastaname="${outname%.*}_pident${pident}_qcovs${qcovs}.fasta"
...
awk 'BEGIN { OFS="\t"; print "qseqid", "qseq", "sallseqid", "sseq", "pident", "qcovs", "evalue", "bitscore" } { print }' "$tempBlastOutputs" > "$outname"
