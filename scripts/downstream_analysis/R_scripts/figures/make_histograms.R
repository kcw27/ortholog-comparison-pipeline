...
save_histograms <- function(df, group_var, outdir, pdf_suffix, reference_length=NA, reference_label=NA, consistent_y_axis = TRUE, export_locus_tags = FALSE) {
...
histograms_by_source <- function(df, outdir, reference_length=NA, reference_label=NA, consistent_y_axis = TRUE, group_var = "category", export_locus_tags = FALSE) {
  make_dir(outdir)
  # Generate histogram PDF for group_var
  save_histograms(df, group_var, outdir, glue("{group_var}_histograms"), 
                  reference_length=reference_length, reference_label=reference_label, consistent_y_axis, export_locus_tags)
  ...
  if (group_var == "category") { # make additional plots for subcategory
    ...
    walk(names(df_categories), ~ save_histograms(df_categories[[.x]], "subcategory", 
                                                ...
                                                consistent_y_axis, export_locus_tags))
